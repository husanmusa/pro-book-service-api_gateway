...
func Load() Config {
	...
	config.ServiceName = cast.ToString(getOrReturnDefaultValue("SERVICE_NAME", "pro-book-service"))
	...
	config.PostgresUser = cast.ToString(getOrReturnDefaultValue("POSTGRES_USER", "husamusa"))
	config.PostgresPassword = cast.ToString(getOrReturnDefaultValue("POSTGRES_PASSWORD", "pass"))
	config.PostgresDatabase = cast.ToString(getOrReturnDefaultValue("POSTGRES_DATABASE", "pro_book"))
	...
	config.BookServiceURL = cast.ToString(getOrReturnDefaultValue("BOOK_SERVICE_URL", "localhost:8082/"))
	return config
}
